...
def test_create_chat_model_uses_foundry_project_scope(monkeypatch, settings_fixture):
    ...
    monkeypatch.setattr("langgraph_fabric_data_agent.llm.factory.DefaultAzureCredential", fake_default_credential)
    monkeypatch.setattr(
        "langgraph_fabric_data_agent.llm.factory.get_bearer_token_provider",
        fake_get_bearer_token_provider,
    )
    monkeypatch.setattr("langgraph_fabric_data_agent.llm.factory.AzureChatOpenAI", FakeAzureChatOpenAI)
