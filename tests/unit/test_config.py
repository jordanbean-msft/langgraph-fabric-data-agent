...
def test_settings_load_log_level_override(monkeypatch):
    ...
    monkeypatch.setenv(
        "LOG_LEVEL_OVERRIDE",
        "langgraph_fabric_data_agent.graph:DEBUG,azure.core:WARNING",
    )

    settings = AppSettings()

    assert settings.log_level_override == "langgraph_fabric_data_agent.graph:DEBUG,azure.core:WARNING"
