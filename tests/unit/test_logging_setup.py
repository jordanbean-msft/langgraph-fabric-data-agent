...
from langgraph_fabric_data_agent.core.logging import _ColorFormatter, configure_logging
...
def restore_logging_state():
    ...
    tracked_loggers = [
        "langgraph_fabric_data_agent.graph",
        "azure.core",
        "test.invalid",
    ]
...
def test_configure_logging_applies_logger_overrides():
    configure_logging(
        "warning",
        "langgraph_fabric_data_agent.graph:DEBUG,azure.core:ERROR",
    )

    assert logging.getLogger().level == logging.WARNING
    assert logging.getLogger("langgraph_fabric_data_agent.graph").level == logging.DEBUG
    assert logging.getLogger("azure.core").level == logging.ERROR


def test_configure_logging_supports_off_override():
    configure_logging("INFO", "langgraph_fabric_data_agent.graph:OFF")

    assert logging.getLogger("langgraph_fabric_data_agent.graph").level > logging.CRITICAL
