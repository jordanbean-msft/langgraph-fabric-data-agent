...
Copy the `password` value from the output and set it as `MICROSOFT_APP_PASSWORD` in the package-local `.env` file for the interface you are configuring, for example `packages/langgraph-fabric-api/.env` or `packages/langgraph-fabric-m365/.env`.
