...
- A `.env.api` file at the repository root with the required server-side environment variables (see [Environment variables](#environment-variables) below).
...
All settings are read from `.env.api` via the API settings model. The following variables are required to run the API server:
...
See [.env.api.example](../.env.api.example) for a full template.
